...
export class Plugin {
    ...
    private async handleMessageAsync(msg: any): Promise<void> {
        ...
        if (msg.config && Object.keys(msg.config).length >= 0 && msg.payload === 'config') {
            ...
            response = { end: await c.EndBlock(msg.end) };
        } else {
            console.log(`WARN: unrecognized msg payload=${msg.payload}, keys=${Object.keys(msg).join(',')} - skipping`);
            return;
        }
...
export function FromAny(any: any): [any | null, string | null, IPluginError | null] {
    ...
    try {
        ...
        if (typeUrl.includes('MessageRegisterIdentity')) {
            return [types.MessageRegisterIdentity.decode(any.value), 'MessageRegisterIdentity', null];
        }
        if (typeUrl.includes('MessageWeaveThread')) {
            return [types.MessageWeaveThread.decode(any.value), 'MessageWeaveThread', null];
        }
        if (typeUrl.includes('MessageBreakThread')) {
            return [types.MessageBreakThread.decode(any.value), 'MessageBreakThread', null];
        }
        if (typeUrl.includes('MessageEndorseSkill')) {
            return [types.MessageEndorseSkill.decode(any.value), 'MessageEndorseSkill', null];
        }
        if (typeUrl.includes('MessageFormGuild')) {
            return [types.MessageFormGuild.decode(any.value), 'MessageFormGuild', null];
        }
        if (typeUrl.includes('MessageJoinGuild')) {
            return [types.MessageJoinGuild.decode(any.value), 'MessageJoinGuild', null];
        }
        ...
        if (typeUrl.includes('MessageAttestContribution')) {
            return [types.MessageAttestContribution.decode(any.value), 'MessageAttestContribution', null];
        }
        if (typeUrl.includes('MessageCastWeightedVote')) {
            return [types.MessageCastWeightedVote.decode(any.value), 'MessageCastWeightedVote', null];
        }
